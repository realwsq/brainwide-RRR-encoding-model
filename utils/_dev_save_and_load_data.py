...
from utils.utils import find_bestdelay_byCC, log_kv, remove_space, make_folder
...
def get_data_folder(*args):
    ...
    for arg in args:
        subfolder += f"{arg}"
    data_folder = make_folder(os.path.join(f"/home/shuqi/EPFL/research/IBL/brainwide-RRR-encoding-model/example1/data/", remove_space(subfolder)))
    return data_folder

def get_processed_data_folder():
    return get_data_folder(data_p['id'])

def data_params(which_areas="all_but_invalid", which_variables="all"):
    ...
    elif which_variables == "three":
        gp["vl"] = ['block', 'side', 'contrast_level']
    elif which_variables == "three_nonlinear":
        gp["vl"] = ['block', 'side', 'contrast_level', 'block_side', 'side_contrast_level']
...
def read_Xy_encoding2(data_p, 
    ...
    Xy_regression = {}
    eid_list = list(np.load("/home/shuqi/EPFL/research/IBL/brainwide-RRR-encoding-model/example1/utils/eid_list.npy"))
...
def _read_Xy(var_list, neural_fname, beh_fname, eid, 
    ...
    if standardize_y:
        ...
        mean_y = np.zeros(sc_mtx_processed.shape[1:])
        std_y = np.ones(sc_mtx_processed.shape[1:])
    ...
    if standardize_X:
        ...
        mean_y = np.zeros(F3d.shape[1:])
        std_y = np.ones(F3d.shape[1:])
...
def iterate_over_eids(Xy_regression, _main_f, **kwargs):
