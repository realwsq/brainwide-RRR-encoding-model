...
from utils.utils import tensor2np, np2tensor, get_device, log_kv
...
def train_model_main(data_all_tv, RRR_model, model_fname,
    ...
    optimizer = torch.optim.LBFGS(RRR_model.model.parameters(),
                            lr=lr,
                            max_iter=max_iter,
...
r2s_cv_best: {eid: nparray of size N_neuron} mean validated r2 as the evaluation of performance for each session and neuron
model: trained model using the whole dataset
"""
def train_model_hyper_selection(data_all, n_comp_list, l2_list, model_fname,
                                lr=1., max_iter=1000, tolerance_grad=1e-7, tolerance_change=1e-4, history_size=50, line_search_fn=None,
    ...
    ####                    and l2 weight
    #### based on the k-fold cross-validated r2
    ...
    if len(l2_list) == 1 and len(n_comp_list) == 1:
        ...
        for l2 in l2_list:
            for n_comp in n_comp_list:
                ...
                ## rewrite the best set of hyperparameters if the mean validated r2 is higher
    ...
    ### get and save the mean validated r2 as the evaluation of performance (fname=f"{model_fname}_R2test.pk")
